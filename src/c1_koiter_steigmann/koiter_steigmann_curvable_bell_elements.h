...
namespace oomph
{
  ...
  {
  public:
    /// Function pointer to basis vectors function which sets basis vectors b1
    /// and b2 (which are in general functions of x)
  ...
  protected:
    ...
    /// set by Rotated_basis_fct_pt
    inline void rotation_matrix_at_node(
  ...
  public:
      ...
      //   Nnodes_to_rotate(0)
    {
      ...
      Bubble_u_internal_index = this->add_internal_data(new Data(0));
#ifdef PARANOID
  ...
  protected:
    ...
    /// Jacobian.
    void shape_and_test_biharmonic(const Vector<double>& s,
                                   Shape& psi,
                                   Shape& psi_b,
                                   Shape& test,
                                   Shape& test_b) const;
  ...
  //          template<unsigned DIM_, unsigned NNODE_1D_>
  //          class PLATE_EQUATIONS>
  class FaceGeometry<KoiterSteigmannC1CurvableBellElement<DIM,
                                                               NNODE_1D,
                                                               BOUNDARY_ORDER,
                                                               PLATE_EQUATIONS>>
  ...
  void KoiterSteigmannC1CurvableBellElement<
    ...
    NNODE_1D,
    BOUNDARY_ORDER,
    PLATE_EQUATIONS>::get_coordinate_x(const Vector<double>& s,
                                       Vector<double>& x) const
  {
    // If the element has been upgraded
    if (Curved_edge == MyC1CurvedElements::none)
