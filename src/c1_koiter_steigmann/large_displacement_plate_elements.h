...
namespace oomph
{
  ...
  class LargeDisplacementPlateEquations
    : public virtual BellElement<DIM, NNODE_1D>
  {
  public:
    ...
    LargeDisplacementPlateEquations()
      : Pressure_fct_pt(0),
    ...
    LargeDisplacementPlateEquations(
      const LargeDisplacementPlateEquations& dummy)
    {
    ...
    /// \short Return the index at which the ith displacement l-type (unknown)
    /// value is stored. In derived multi-physics elements, this function should
    /// be overloaded to reflect the chosen storage scheme. Note that these
    /// equations require that the unknown is always stored at the same index at
    /// each node.
    inline unsigned u_index_koiter_model(const unsigned& l,
    ...
    /// the strength of the pressure function might be determined by
    /// another system of equations.
    inline void get_d_pressure_biharmonic_dn(
      const unsigned& ipt,
      const Vector<double>& x,
      const Vector<double>& u,
      const DenseMatrix<double>& grad_u,
    ...
    /// the strength of the pressure function might be determined by
    /// another system of equations.
    inline void get_d_pressure_biharmonic_dr(
      const unsigned& ipt,
      const Vector<double>& x,
      const Vector<double>& u,
      const DenseMatrix<double>& grad_u,
    ...
    virtual double metric_tensor_determinant(
      const DenseMatrix<double>& interpolated_drdxi) = 0;
    ...
    // Return the determinant of the metric tensor
    virtual void d_metric_tensor_determinant_du_unknown(
      const DenseMatrix<double>& g_tensor,
      const RankFourTensor<double>& d_g_tensor_du_unknown,
      DenseMatrix<double>& d_metric_tensor_determinant_du_unknown) = 0;

    // Return the interpolated unit normal
    virtual void fill_in_unit_normal(
      const DenseMatrix<double>& interpolated_drdxi,
      Vector<double>& normal) = 0;

    // Return the interpolated unit normal
    virtual void fill_in_d_unit_normal_du_unknown(
      const DenseMatrix<double>& interpolated_drdxi,
      const DenseMatrix<double>& g_tensor,
      const RankFourTensor<double>& d_g_tensor_du_unknown,
      RankThreeTensor<double>& d_normal_du_unknown) = 0;

    // Get the (Green Lagrange) strain tensor
    virtual void fill_in_strain_tensor(
      const DenseMatrix<double>& interpolated_dudxi,
      DenseMatrix<double>& strain) = 0;

    // Get the (Green Lagrange) strain tensor
    virtual void fill_in_d_g_tensor_du_unknown(
      const DenseMatrix<double>& interpolated_dudxi,
      RankFourTensor<double>& d_g_tensor_du_unknown) = 0;

    // Get the (Green Lagrange) strain tensor
    virtual void fill_in_d_strain_tensor_du_unknown(
      const DenseMatrix<double>& interpolated_dudxi,
      RankFourTensor<double>& d_epsilon_tensor_du_unknown) = 0;
    ...
    virtual void fill_in_curvature_tensor(
      const Vector<double>& unit_normal,
      const DenseMatrix<double>& interpolated_d2rdxi2,
      DenseMatrix<double>& curvature) = 0;

    // Fill in the curvature tensor
    virtual void fill_in_d_curvature_tensor_du_unknown(
      const Vector<double>& unit_normal,
      const DenseMatrix<double>& interpolated_d2rdxi2,
    ...
    virtual void fill_in_moment_tensor(
      const DenseMatrix<double>& interpolated_d2rxi2,
      const Vector<double>& unit_normal,
      const DenseMatrix<double>& curvature,
      RankThreeTensor<double>& moment) = 0;

    // Fill in the moment tensor
    virtual void fill_in_d_moment_tensor_du_unknown(
      const DenseMatrix<double>& interpolated_d2rxi2,
      const Vector<double>& unit_normal,
      const DenseMatrix<double>& curvature,
...
difference stress is not yet defined so an error has been thrown.",
    ...
    virtual void fill_in_total_tension(
      ...
      const DenseMatrix<double>& interpolated_dudxi,
      const DenseMatrix<double>& interpolated_d2rdxi2,
      DenseMatrix<double>& tension_vectors) = 0;

    virtual void d_fill_in_total_tension_du_unknown(
      const DenseMatrix<double>& stress,
      const RankThreeTensor<double>& christoffel_tensor,
      const RankThreeTensor<double>& moment_tensors,
      const DenseMatrix<double>& interpolated_dudxi,
      ...
      RankFourTensor<double>& d_tension_vectors_du_unknown) = 0;

    // Get the Christtoffel tensor of the second kind
    // \Gamma_{\alpha\beta\gamma} = ( E_{\alpha\beta,gamma} +
    // E_{\gamma\alpha,\beta}
    //   - E_{\beta\gamma,\alpha})
    virtual void fill_in_second_christoffel_tensor(
      const DenseMatrix<double>& interpolated_dudxi,
      const DenseMatrix<double>& interpolated_d2udxi2,
    ...
    // E_{\gamma\alpha,\beta}
    //   - E_{\beta\gamma,\alpha})
    virtual void fill_in_d_second_christoffel_tensor_dui_unknown(
      const DenseMatrix<double>& interpolated_dudxi,
      const DenseMatrix<double>& interpolated_d2udxi2,
    ...
    inline void interpolated_u_koiter_plate(
      const Vector<double>& s, Vector<Vector<double>>& interpolated_u) const
    {
      ...
      DShape dpsi_dxi(n_node, n_position_type, DIM),
        ...
        dpsi_b_dxi(n_b_node, n_b_position_type, DIM),
        dtest_b_dxi(n_b_node, n_b_position_type, DIM),
        d2psi_dxi2(n_node, n_position_type, 3),
        d2test_dxi2(n_node, n_position_type, 3),
        d2psi_b_dxi2(n_b_node, n_b_position_type, 3),
        d2test_b_dxi2(n_b_node, n_b_position_type, 3);
